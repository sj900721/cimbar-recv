...
var RecvWorker = function () {
  return {
    on_frame: function (data) {
      ...
      var len = Module._cimbard_scan_extract_decode(
        RecvWorker.imgBuff().byteOffset, width, height, type,
        fountainBuff.byteOffset, fountainBuff.length
      );
      if (len <= 0) {
        var errmsg = RecvWorker.get_error();
        let msg = { res: len + " " + errmsg };
        if (len == 0) msg.nodata = true;
        else if (len == -3) msg.failed_extract = true;
        else msg.error = true;
    ...
    get_error: function () {
      ...
      if (errlen > 0) {
        ...
        return new TextDecoder().decode(errview);
      }
      return "";
    },
